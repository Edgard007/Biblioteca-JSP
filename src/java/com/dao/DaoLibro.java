...
public class DaoLibro extends Conexion {
    ...
    public List<Genero> listarGenero() throws Exception
    {
     ...
     //Este metodo se Ocupa en Compras Administrativas
        
        //Sacar Cantidad Existente
         public int sacarCant(int id) throws Exception
         {
         this.conectar();
         ResultSet rs2;
     ...
     public int modificarLb(int id,int n) throws Exception{
        int flag;
        PreparedStatement pstmt;
        String sql;
        
        int n2=this.sacarCant(id);
        int cant=n+n2;
        ...
        try{
            if(id>0)
            {
                ...
                {
                 this.conectar();
                sql = "update libro set cantidad=? WHERE idLibro=?";
